...
reconstructs the court geometry, and computes shot speed, shot count and
player movement speed, rendered on an annotated video with a top-down
minimap.

## Pipeline

video -> court keypoints (YOLO26-pose) -> homography -> player tracking
(YOLO26) -> ball detection (YOLO26 fine-tuned) -> Kalman smoothing ->
projection to metric court space -> shot detection -> statistics -> rendering.
...
tennisvision/            library code
    ...
    events.py            shot/hit detection
    analytics.py         shot and movement statistics
    viz.py               frame annotations and top-down minimap
    video.py             video I/O
